...
import column from 'app/data/columns/mailHistory';
...
const MailHistory = () => {
  ...
  const handleGetMails = async () => {
    try {
      ...
      if (payload.status === 200 && payload?.data) {
        await dispatch(setMails(payload.data));
        await dispatch(setSearchedFlag({ mails: true }));
      } else {
  ...
  useEffect(() => {
    const { popols, mails } = searchedFlag;
    if (!popols) {
      handleGetPopols();
    }
    if (!mails) {
